...
class ResetSeekableStreamTestCase:
...
seekable_test_cases = [
    # bytes -> BytesIO
    ResetSeekableStreamTestCase(lambda: b"0123456789", None, False, b"0123456789"),
    # str -> BytesIO
    ResetSeekableStreamTestCase(lambda: "0123456789", None, False, b"0123456789"),
    # BytesIO directly
    ResetSeekableStreamTestCase(lambda: io.BytesIO(b"0123456789"), None, False, b"0123456789"),
    # BytesIO directly with offset
    ResetSeekableStreamTestCase(lambda: io.BytesIO(b"0123456789"), 4, False, b"456789"),
    # StringIO
    ResetSeekableStreamTestCase(lambda: io.StringIO("0123456789"), None, False, b"0123456789"),
    # Non-seekable
    ResetSeekableStreamTestCase(lambda: ResetSeekableStreamTestCase.create_non_seekable_stream(b"0123456789"),
                                None, True, b"0123456789")
]


@pytest.mark.parametrize('test_case', seekable_test_cases)
def test_rewind_seekable_stream_on_retryable_error_response(test_case: ResetSeekableStreamTestCase):
    received_requests = []
...
@pytest.mark.parametrize('test_case', seekable_test_cases)
def test_rewind_seekable_stream_on_retryable_exception(test_case: ResetSeekableStreamTestCase):
    failure_count = 2
